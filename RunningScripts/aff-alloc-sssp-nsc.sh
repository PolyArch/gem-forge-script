...
run_ssp () {
    ...
    python Driver.py $Benchmark $SimTrace -t $trans \
        ...
        -s -j $parallel \
        # --no-job-log \
        # --gem5-debug ISAStreamEngine,LSQ,LSQUnit,Commit,Writeback,ProtocolTrace,RubySlicc,RubyQueue,RubyGenerated --gem5-debug-start 42930468000 --gem5-max-ticks 43100001000 2>&1 | tee /benchmarks/ruby.log
        # --gem5-variant fast \
        # --perf-command \
