...
def generateGem5Env():
    ld_library_paths = [
        C.LLVM_LIB_PATH, # Mainly for libomp
        os.path.join(C.GEM_FORGE_TRANSFORM_BUILD_PATH, 'src', 'affinity_alloc'), # Affinity allocator
    ]
    ld_library_path = ':'.join(ld_library_paths)
    gem5_env_fn = os.path.join(C.GEM_FORGE_DRIVER_PATH, 'Utils', 'Gem5Env.sh')
    with open(gem5_env_fn, 'w') as f:
        f.write('# This is automatcially generated by Driver.py.\n')
        f.write(f'LD_LIBRARY_PATH={ld_library_path}\n')
